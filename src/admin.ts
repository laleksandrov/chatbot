...
async function dashboard(
  ...
  const clientRows = clients.map((client) => {
    const currentRoles = apiRoles.map((role) => `<label class="check"><input type="checkbox" name="roles" value="${role}"${client.roles.includes(role) ? " checked" : ""}>${role}</label>`).join("");
    const currentProfiles = assistantProfiles.map((profile) => `<label class="check"><input type="checkbox" name="profiles" value="${profile}"${client.allowedProfiles.includes(profile) ? " checked" : ""}>${profile}</label>`).join("");
    const defaultOptions = assistantProfiles.map((profile) => `<option${profile === client.defaultProfile ? " selected" : ""}>${profile}</option>`).join("");
    return `<tr><td><strong>${escapeHtml(client.name)}</strong><div class="muted">${escapeHtml(client.tenantId)}</div></td><td><code>${escapeHtml(client.keyPrefix)}…</code></td><td>${client.roles.map(escapeHtml).join("<br>")}</td><td>${client.allowedProfiles.map(escapeHtml).join("<br>")}<div class="muted">по подразбиране: ${escapeHtml(client.defaultProfile)}</div></td><td>${client.active ? "Активен" : "Спрян"}<div class="muted">${client.lastUsedAt?.toISOString() ?? "неизползван"}</div></td><td><form class="inline" method="post" action="/admin/api-clients/${client.id}/toggle"><input type="hidden" name="csrf" value="${csrf}"><input type="hidden" name="active" value="${client.active ? "false" : "true"}"><button class="${client.active ? "danger" : "secondary"}">${client.active ? "Спри" : "Активирай"}</button></form></td></tr><tr><td colspan="6"><details><summary>Промени права и режими</summary><form method="post" action="/admin/api-clients/${client.id}/access"><input type="hidden" name="csrf" value="${csrf}"><fieldset><legend>Права</legend>${currentRoles}</fieldset><fieldset><legend>Режими</legend>${currentProfiles}</fieldset><label>По подразбиране</label><select name="defaultProfile">${defaultOptions}</select><p><button>Запази правата</button></p></form></details></td></tr>`;
  }).join("");
  const userRows = users.map((user) => `<tr><td>${escapeHtml(user.email)}</td><td>Потребител</td><td>${user.active ? "Активен" : "Спрян"}</td><td><form class="inline" method="post" action="/admin/users/${user.id}/access"><input type="hidden" name="csrf" value="${csrf}"><input type="hidden" name="active" value="${user.active ? "false" : "true"}"><button class="${user.active ? "danger" : "secondary"}">${user.active ? "Спри" : "Активирай"}</button></form></td></tr>`).join("");
  return page("Администрация", `<header><strong>Leon Chatbot · Администрация</strong><span>${escapeHtml(session.user.email)}</span><form method="post" action="/admin/logout"><input type="hidden" name="csrf" value="${csrf}"><button class="secondary">Изход</button></form></header><main>${notice ? `<div class="notice">${escapeHtml(notice)}</div>` : ""}${generatedKey ? `<section class="card"><h2>Нов API ключ — копирайте го сега</h2><p>След затваряне няма да може да бъде показан отново.</p><div class="secret">${escapeHtml(generatedKey)}</div></section>` : ""}<section class="card"><h2>API клиенти</h2><table><thead><tr><th>Име / tenant</th><th>Ключ</th><th>Права</th><th>Режими</th><th>Статус</th><th></th></tr></thead><tbody>${clientRows || "<tr><td colspan=6>Няма клиенти.</td></tr>"}</tbody></table></section><div class="grid"><section class="card"><h2>Нов API клиент</h2><form method="post" action="/admin/api-clients"><input type="hidden" name="csrf" value="${csrf}"><label>Име</label><input name="name" required placeholder="EasyStart public"><label>Tenant ID</label><input name="tenantId" required placeholder="easystart"><fieldset><legend>Права</legend>${roleChecks}</fieldset><fieldset><legend>Разрешени режими</legend>${profileChecks}</fieldset><label>Режим по подразбиране</label><select name="defaultProfile">${assistantProfiles.map((p) => `<option>${p}</option>`).join("")}</select><p><button>Създай и покажи ключа</button></p></form></section><section class="card"><h2>Нов потребител</h2><p class="muted">Администраторският достъп се определя само от Forge environment.</p><form method="post" action="/admin/users"><input type="hidden" name="csrf" value="${csrf}"><label>Имейл</label><input type="email" name="email" required><label>Парола (мин. 12 знака)</label><input type="password" name="password" minlength="12" required><p><button>Създай</button></p></form></section></div><section class="card"><h2>Потребители</h2><table><thead><tr><th>Имейл</th><th>Роля</th><th>Статус</th><th></th></tr></thead><tbody>${userRows}</tbody></table></section></main>`);
}
...
export async function registerAdminRoutes(app: FastifyInstance, repository: AccessAdminRepository, secureCookies: boolean): Promise<void> {
  ...
  app.post<{ Params: { id: string } }>("/admin/api-clients/:id/access", async (request, reply) => {
    const session = await requireSession(request, reply, repository); if (!session) return;
    assertCsrf(request, session); const body = form(request);
    const roles = selectedValues(body.roles).filter((v): v is ApiRole => apiRoles.includes(v as ApiRole));
    const profiles = selectedValues(body.profiles).filter((v): v is AssistantProfile => assistantProfiles.includes(v as AssistantProfile));
    const defaultProfile = String(body.defaultProfile ?? "") as AssistantProfile;
    if (!roles.length || !profiles.length || !profiles.includes(defaultProfile)) throw new HttpError(400, "INVALID_API_CLIENT_ACCESS", "Изберете валидни права и режим по подразбиране.");
    await repository.updateApiClientAccess(request.params.id, { roles, allowedProfiles: profiles, defaultProfile });
    return reply.redirect("/admin");
  });
  app.post("/admin/users", async (request, reply) => {
    const session = await requireSession(request, reply, repository); if (!session) return;
    assertCsrf(request, session); const body = form(request);
    await repository.createUser({ email: String(body.email ?? ""), password: String(body.password ?? ""), isAdmin: false });
    return reply.redirect("/admin");
  });
  app.post<{ Params: { id: string } }>("/admin/users/:id/access", async (request, reply) => {
    const session = await requireSession(request, reply, repository); if (!session) return;
    assertCsrf(request, session); const body = form(request);
    await repository.setUserAccess(request.params.id, { active: String(body.active) === "true", isAdmin: false });
    return reply.redirect("/admin");
  });
}
